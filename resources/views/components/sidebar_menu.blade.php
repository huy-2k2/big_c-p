<ul>
    ...
    <li>
        <a  @if(!isset($item['children'])) href="{{ $item['link'] }}" @endif  class="relative flex items-center w-full px-1 py-3 cursor-pointer sidebar-item gap-x-2 group">
            ...
            <span class="font-semibold text-gray-600 capitalize sidebar-hidden text-md group-hover:text-gray-900 whitespace-nowrap">{{ $item['text'] }}</span>
            @isset($item['children'])
        ...
        @isset($item['children'])
            <ul class="hidden overflow-hidden duration-300 sidebar-children sidebar-hidden">
               ...
               <li>
                    <a class="whitespace-nowrap block pl-[50px] py-2 capitalize text-gray-500 text-[14px] hover:text-gray-800"  href={{ $children['link'] }}>{{ $children['text'] }}</a>
                </li>
               @endforeach
            </ul>
        @endisset
